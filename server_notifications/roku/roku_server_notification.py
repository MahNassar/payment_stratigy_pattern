...
class RokuServer(IPaymentServerGateway):

    def get_subscription_info(self, request):
        subscription_info = SubscriptionInfo()
        # envelope = json.loads(request.data.decode('utf-8'))
        subscription_info.server_payment_gateway = "roku"
        # Map Data ...

        envelope = request
        if envelope:
            subscription_info.original_request = envelope
            subscription_info.purchase_token = envelope['originalTransactionId']
            subscription_info.plan_package = envelope['productCode']

            notification_type = envelope["transactionType"]
            if notification_type == "Cancellation":  # 2. An active subscription was renewed.
